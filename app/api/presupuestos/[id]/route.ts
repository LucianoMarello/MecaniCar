...
export async function GET(_request: Request, { params }: Contexto) {
  ...
  if (resultado.resultado === "PROHIBIDO") return NextResponse.json({ error: "No puede acceder a este presupuesto" }, { status: 403 });
  const total = resultado.presupuesto.detalles.reduce((suma, detalle) => suma + Number(detalle.precioAplicado), 0);
